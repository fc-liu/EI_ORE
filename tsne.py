...
def x2p_torch(X, tol=1e-5, perplexity=30.0):
    ...
    P = torch.zeros(n, n).to(opt.cuda)
    beta = torch.ones(n, 1).to(opt.cuda)
    logU = torch.log(torch.tensor([perplexity]).to(opt.cuda))
    ...
    for i in range(n):
        ...
        betamin = None
        betamax = None
        Di = torch.Tensor(D[i, n_list[0:i]+n_list[i+1:n]]).to(opt.cuda)
...
def tsne(X, no_dims=2, initial_dims=50, perplexity=30.0):
    ...
    eta = 500
    min_gain = 0.01
    Y = torch.randn(n, no_dims).to(opt.cuda)
    dY = torch.zeros(n, no_dims).to(opt.cuda)
    iY = torch.zeros(n, no_dims).to(opt.cuda)
    gains = torch.ones(n, no_dims).to(opt.cuda)
    ...
    P = torch.max(P, torch.tensor([1e-21]).to(opt.cuda))

    # Run iterations
    tq = tqdm(range(max_iter))
    for iter in tq:
